...
#define ASSERT_NO_ZEROS(args) { \
    ...
    for (size_t i = 1; i < ScamSeq_len(args); i++) { \
...
ScamVal* typecheck_all(char* name, ScamSeq* args, unsigned int min, int type_we_need) {
    size_t n = ScamSeq_len(args);
    if (n < min) {
